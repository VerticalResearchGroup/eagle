...
from common.QSL import AudioQSL, AudioQSLInMemory 
from parts.preprocessing import AudioPreprocessing
...
def main():
    ...
    #                               dataset_path=args.dataset_pathir,
    ...
    #EAGLE CPU conversion
    #train_feat_proc.cuda()
    #val_feat_proc.cuda()
    
    # qsl = AudioQSLInMemory(args.dataset_dir,
    qsl = AudioQSL(args.dataset_dir,
                            args.train_manifests[0],
                            # args.train_manifests,
                            tokenizer.charset,
                            16000,
                            None)
  
    feature_config = cfg['input_train']['filterbank_features']
    audio_preprocessor = AudioPreprocessing(**feature_config)
    query_sample = np.array(list(qsl.sample_id_to_sample.keys()))
    np.random.shuffle(query_sample)
    steps_per_epoch = len(query_sample) // args.grad_accumulation_steps
    ...
    # EAGLE: Change FusedLAMB optimizer because it was using GPU to Lamvb which doesn't use GPU
    # optimizer = FusedLAMB(betas=(args.beta1, args.beta2), eps=opt_eps, max_grad_norm=args.clip_norm, **kw)
    optimizer = Lamb(betas=(args.beta1, args.beta2), eps=opt_eps, **kw)
    ...
    print_once("Epochs: " + str(args.epochs) + " Steps For Grad Accumulation in epoch: " + str(args.grad_accumulation_steps) + " Batch Size: " + str(batch_size))
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        for batch in range(0,len(query_sample),batch_size):
            print("Batch: ",batch)
            ...
            waveform, waveform_length, txt, txt_lens = qsl[query_sample[batch:batch+batch_size]]
            # assert waveform.ndim == 1
            # waveform_length = np.array(waveform.shape[1], dtype=np.int64)
            ...
            with torch.no_grad():
                ...
                txt_lens = torch.from_numpy(txt_lens)
                feats, feat_lens = audio_preprocessor.forward((waveform, waveform_length))
                assert feats.ndim == 3
                assert feat_lens.ndim == 1
            ...
            # print("Audio Lens Shape: ",waveform_length.shape)
            # print(waveform.device)
            # print(waveform_length.device)
            # print(audio_lens)
            # print("Feats Shape: ",feats.shape)
            # print("Feat Lens Shape: ",feat_lens.shape)
            # print(feat_lens)
            # print("Txt Shape: ",txt.shape)
            # print(txt)
            # print("Txt Lens Shape: ",txt_lens.shape)
            feats = feats.cpu()
            feat_lens = feat_lens.cpu()
            txt = txt.cpu()
            txt_lens = txt_lens.cpu()
            
            # print(feats.is_cuda, feat_lens.is_cuda, txt.is_cuda, txt_lens.is_cuda)
            log_probs, log_prob_lens = model(feats, feat_lens, txt, txt_lens)
            # print("Log probs Shape: ",log_probs.shape)
            # print("Log probs Lens Shape: ",log_prob_lens.shape)
            loss = loss_fn(log_probs[:, :log_prob_lens.max().item()],
                                      log_prob_lens, txt, txt_lens)
                                    #   txt, log_prob_lens, txt_lens)
            ...
            if torch.isnan(loss).any():
                ...
                step_utts += batch_size * world_size
                # epoch_utts += batch[0].size(0) * world_size
                epoch_utts += batch_size * world_size
    ...
    # EAGLE: Evaluation part is not implemented for CPU version
