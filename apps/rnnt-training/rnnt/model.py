...
class RNNT(nn.Module):
    ...
    def __init__(self, n_classes, in_feats, enc_n_hid,
        ...
        # self.joint_net = nn.Sequential(OrderedDict([
        #   ('relu', torch.nn.ReLU(inplace=True)),
        #   ('dropout', torch.nn.Dropout(p=joint_dropout)),
        #   ('linear', torch.nn.Linear(joint_n_hid, n_classes))
        # ]))
        # self.joint_net = nn.ModuleList([
    ...
    def joint(self, f, g):
        ...
        # print('g: ',g.shape)
        # print('f+g: ',(f+g).shape)
        # print(self.joint_net)
        # for idx, m in enumerate(self.joint_net.modules()):
        #     print(idx, '->', m)
        # hidden = f+g
        # for idx, layer in enumerate(self.joint_net):
        #     print('Joint Net ID: ',idx)
        #     hidden = layer(hidden)
        #     print('Finished Joint Net: ',layer)
        # print('Joint Net Completed')
        # res = hidden
