...
from runtime.logging import mllog_event, mllog_start, mllog_end, CONSTANTS, log_env_info
...
def train(flags, model, train_loader, val_loader, loss_fn, score_fn, device, callbacks, is_distributed):
    ...
    for epoch in range(1, flags.epochs + 1):
        ...
        for iteration, batch in enumerate(tqdm(train_loader, disable=(rank != 0) or not flags.verbose)):
            ...
            #edit
            # tr = torch.jit.trace(model, image)
            # tr.eval()
            # ftr = torch.jit.freeze(tr)
            # print(ftr.inlined_graph)
            ...
            #edit
            mllog_event(key=CONSTANTS.STATUS,
                        metadata={CONSTANTS.EPOCH_NUM: iteration},
                        sync=False)
